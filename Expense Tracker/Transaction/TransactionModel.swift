...
struct Transaction: Identifiable, Decodable, Hashable {
    ...
    var day: String {
        dateParsed.formatted(.dateTime.year().month().day(.defaultDigits))
    }
    
    var year: String {
        dateParsed.formatted(.dateTime.year(.defaultDigits))
    }
}
...
extension Category {
    ...
    static let moviesAndDVDs = Category(id: 301,
                                        name: "Movies & DVDs", icon: .film, mainCategoryId: 3)
    ...
    static let rent = Category(id: 601, name: "Rent",
                               icon: .house_user, mainCategoryId: 6)
